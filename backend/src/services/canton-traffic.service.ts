...
export class CantonTrafficService {
  ...
  validateMetadata(metadata: any): metadata is TrafficBuyMetadata {
    return (
      ...
      typeof metadata.validatorPartyId === 'string' &&
      typeof metadata.trafficAmountBytes === 'number' &&
      typeof metadata.domainId === 'string' &&
      metadata.validatorPartyId.length > 0 &&
      metadata.domainId.length > 0 &&
      metadata.trafficAmountBytes > 0
    );
  }
