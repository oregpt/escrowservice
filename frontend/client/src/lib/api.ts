...
export const escrows = {
  ...
  fund: (id: string, notes?: string) =>
    apiFetch<Escrow>(`/escrows/${id}/fund`, {
      method: 'POST',
      body: JSON.stringify({ notes }),
    }),

  confirm: (id: string, notes?: string) =>
    apiFetch<Escrow>(`/escrows/${id}/confirm`, {
      method: 'POST',
      body: JSON.stringify({ notes }),
    }),
...
export const attachments = {
  upload: async (
    ...
    if (options?.confirmationStep) {
      formData.append('confirmationStep', options.confirmationStep);
    }
    if (options?.holdUntilCompletion) {
      formData.append('holdUntilCompletion', 'true');
    }
    if (options?.notes) {
...
export const serviceTypes = {
  ...
  getAvailable: () => apiFetch<ServiceType[]>('/settings/org/service-types/available'),
};

// ===== ORG SERVICE TYPE SETTINGS (org admin) =====
export interface OrgServiceTypeSetting {
  id: string;
...
export const orgServiceTypes = {
  // Get service types with org status (for org admin UI)
  getForOrg: (orgId: string) => apiFetch<(ServiceType & { orgEnabled: boolean })[]>(`/settings/org/${orgId}/service-types`),
  // Toggle service type enabled/disabled for org
  toggle: (orgId: string, serviceTypeId: string, isEnabled: boolean) =>
    apiFetch<OrgServiceTypeSetting>(`/settings/org/${orgId}/service-types/${serviceTypeId}`, {
      method: 'PATCH',
