...
export default function EscrowDetail() {
  ...
  const arbiterCancelMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setArbiterReason('');
      toast({ title: "Deal Canceled", description: "The deal has been canceled by arbiter." });
    },
    onError: (error) => {
  ...
  const arbiterForceCompleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setArbiterReason('');
      toast({ title: "Deal Completed", description: "Funds have been released to the provider by arbiter." });
    },
    onError: (error) => {
  ...
  const handleAccept = async () => {
    try {
      ...
      toast({ title: "Deal accepted", description: "You have accepted this deal request." });
    } catch (error) {
  ...
  const handleCancel = async () => {
    try {
      ...
      toast({ title: "Deal canceled", description: "The deal has been canceled." });
    } catch (error) {
      toast({ title: "Error", description: error instanceof Error ? error.message : "Failed to cancel", variant: "destructive" });
    }
  };
  ...
  const getNextAction = () => {
    switch (escrow.status) {
      ...
      case 'PENDING_ACCEPTANCE':
        return isPartyB
          ? { actor: 'You', action: 'need to accept this deal request.' }
          : { actor: 'Counterparty', action: 'needs to accept this deal request.' };
      case 'PENDING_FUNDING':
        return isPartyA
          ? { actor: 'You', action: 'need to fund the deal.' }
          : { actor: 'Originator', action: 'needs to fund the deal.' };
      case 'FUNDED':
        return isPartyB
      ...
      case 'COMPLETED':
        return { actor: 'Complete', action: 'Deal has been completed successfully.' };
      case 'CANCELED':
        return { actor: 'Canceled', action: 'This deal was canceled.' };
      default:
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {/* Upload section - only show for active deals and parties involved */}
                {!['COMPLETED', 'CANCELED', 'EXPIRED'].includes(escrow.status) && (isPartyA || isPartyB) && (
                  <AttachmentUpload escrowId={escrow.id} />
                )}
            ...
            <Card className="border-emerald-100 bg-emerald-50/30">
              ...
              <CardContent className="space-y-4">
                ...
                {(escrow.status === 'CREATED' || escrow.status === 'PENDING_ACCEPTANCE') && isPartyB && (
                  <div className="flex items-center justify-between bg-white p-4 rounded-lg border border-emerald-100 shadow-sm">
                    <div className="space-y-1">
                      <h4 className="font-medium">Accept Deal</h4>
                      <p className="text-xs text-muted-foreground">Accept this deal request to proceed.</p>
                    </div>
                ...
                {escrow.status === 'PENDING_FUNDING' && isPartyA && (
                  <div className="flex items-center justify-between bg-white p-4 rounded-lg border border-emerald-100 shadow-sm">
                    <div className="space-y-1">
                      <h4 className="font-medium">Fund Deal</h4>
                ...
                {escrow.status === 'COMPLETED' && (
                  <div className="flex items-center gap-3 bg-white p-4 rounded-lg border border-emerald-100 shadow-sm">
                    ...
                    <div>
                      <h4 className="font-medium">Deal Completed</h4>
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-slate-900 text-white border-slate-800">
              <CardContent className="pt-6">
                <div className="text-sm text-slate-400 mb-1">Deal Balance</div>
        ...
        <Dialog open={arbiterCancelOpen} onOpenChange={setArbiterCancelOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                As the arbiter, you can cancel this deal and refund the funds. This action cannot be undone.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Textarea
                  id="cancel-reason"
                  placeholder="Explain why this deal is being canceled..."
                  value={arbiterReason}
