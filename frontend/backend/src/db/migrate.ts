...
-- Organization Service Type Settings (org-level restrictions on platform service types)
-- Platform admins control is_active on service_types table
-- Org admins can further restrict which service types are available to their org
CREATE TABLE IF NOT EXISTS org_service_type_settings (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    ...
    created_at TIMESTAMP DEFAULT NOW(),
    updated_at TIMESTAMP DEFAULT NOW(),
    UNIQUE(organization_id, service_type_id)
);
...
-- Add hold_until_completion column to attachments for escrow visibility
-- When true, attachment is hidden from other party until both parties confirm
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attachments' AND column_name = 'hold_until_completion') THEN
        ALTER TABLE attachments ADD COLUMN hold_until_completion BOOLEAN DEFAULT false;
    END IF;
END $$;

-- Add confirmation_step column to attachments to track which step it's associated with
-- Values: FUNDING, PARTY_B_CONFIRM, PARTY_A_CONFIRM, or NULL for general attachments
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attachments' AND column_name = 'confirmation_step') THEN
        ALTER TABLE attachments ADD COLUMN confirmation_step VARCHAR(30);
    END IF;
END $$;

-- Add notes column to attachments for accompanying text
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'attachments' AND column_name = 'notes') THEN
